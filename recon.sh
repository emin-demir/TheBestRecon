...
for domain in "${DOMAINS[@]}"; do
    ...
    if $URL_DISCOVERY; then
        bash modules/url_discovery.sh "$domain" -o "$domain_output_dir"
    fi
    ...
    if $JS_DISCOVERY || $PARAM_DISCOVERY; then
        ...
        bash modules/js_and_param_discovery.sh "$domain" -o "$domain_output_dir" $JS_FLAG $PARAM_FLAG
    fi
    
    # 6. Port Scanning
    if $PORT_SCAN; then
        bash modules/port_scan.sh "$domain" -o "$domain_output_dir"
    fi
    ...
    if $VULN_SCAN; then
        bash modules/vuln_scan.sh "$domain" -o "$domain_output_dir"
    fi
done
