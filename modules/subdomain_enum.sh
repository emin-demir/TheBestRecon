...
if ! $SKIP_DNSX; then
    ...
    if command -v dnsx &> /dev/null; then
        ...
        cat "$SUBDOMAIN_ENUM_DIR/dnsx_raw_${DOMAIN}.txt" | cut -d ' ' -f3 | sed 's/\[\(.*\)\]/\1/g' | sort -u > "$SUBDOMAIN_ENUM_DIR/dns_ips_${DOMAIN}.txt"
        
        LIVE_SUBDOMAINS=$(wc -l < "$SUBDOMAIN_ENUM_DIR/dnsx_live_subdomains_${DOMAIN}.txt")
        IP_COUNT=$(wc -l < "$SUBDOMAIN_ENUM_DIR/dns_ips_${DOMAIN}.txt")
...
echo -e "${GREEN}###------- Subdomain enumeration tamamlandı: $DOMAIN -------###${NC}"